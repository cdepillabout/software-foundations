...
Fixpoint re_contains_star {T} (re : reg_exp T) : Prop :=
  match re with
  | App re1 re2 => re_contains_star re1 \/ re_contains_star re2
  | Union re1 re2 => re_contains_star re1 \/ re_contains_star re2
  | Star _ => True
  | _ => False
  end.

Lemma pumping_re_contains_star T : forall s (re : reg_exp T), 
  s =~ re -> pumping_constant re <= length s -> re_contains_star re.
Proof.
  intros s re H. induction H; simpl; intros; auto.
  - inversion H.
  - inversion H; subst; clear H. inversion H1. 
  - rewrite app_length in H1.
    apply add_le_cases in H1 as []; auto.
  - apply plus_le in H0 as []; auto.
  - apply plus_le in H0 as []; auto.
  Qed.
...
Lemma napp_one_more :
  ...
  s ++ napp m s =~ Star re ->
  s ++ s ++ napp m s =~ Star re.
Proof.
  intros T; induction m; simpl; intros.
  - rewrite app_nil_r. rewrite app_nil_r in H.
    inversion H; subst.
    + auto.
    +   
  (* 
  intros T m s re H. remember (Star re) as starry.
  ...
  *)
  (* intros T m s. generalize dependent m. induction s; intros; auto.
  *)
  (* intros T m s re. generalize dependent s. generalize dependent m.
  induction re; intros; auto.
  - simpl in H. inversion H; subst.
    + rewrite <- H1 in H. admit.
    + inversion H2.
  - simpl in H. inversion H; subst.
    + admit.
    + inversion H2; subst. simpl in H1. simpl.  
  *)
*)
...
Proof.
  ...
  - destruct n.
    ...
    + right. left. split; auto.
      unfold ">", "<". clear H. clear q. induction n; auto.
  - inversion H; subst.
    + replace (S (p + q)) with (p + S q) in H.
      * apply IHp in H as [|[[]|]]; clear IHp.
        -- left. admit.
        -- right.
  *)
  (* induction on H does NOT seem to work *)
...
Proof.
  ...
  - simpl. intros. apply plus_le in H as [].
    ...
    exists x,x0,x1; split; try split; auto; intros; auto using MUnionL.
  - simpl. intros. apply plus_le in H as [].
