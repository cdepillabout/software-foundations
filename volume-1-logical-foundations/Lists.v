...
Fixpoint filterlist (f: nat -> bool) (l:natlist) : natlist :=
  match l with
  | [] => []
  | h :: t => if f h then h :: filterlist f t else filterlist f t
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | [] => []
  | n :: t => if v =? n then t else n :: remove_one v t
  end.
...
Definition remove_all (v:nat) : bag -> bag := filterlist (compose negb (eqb v)).
...
Theorem eqb_ref : forall x, x =? x = true.
Proof.
  intros x.
  induction x.
  - simpl. reflexivity.
...
Theorem bag_theorem : forall x bag, count x bag + 1 = count x (add x bag).
Proof.
  intros x bag.
  unfold add.
  unfold count.
  ...
  rewrite eqb_ref.
  simpl.
  assert (H: forall z, z + 1 = S z). { induction z. reflexivity. simpl. rewrite IHz. reflexivity. }
  rewrite H.
  reflexivity.
Qed.
...
Lemma count_first_match: forall (s : bag) n, count n (n :: s) = S (count n s).
Proof.
  intros s n. unfold count. simpl. rewrite eqb_ref. simpl. reflexivity. Qed.
...
Proof.
  intros s. unfold count. induction s as [|h s' IHs'].
  - simpl. reflexivity.
  - simpl. destruct h as [|n] eqn:F.
...
Theorem rev_injective_reverse : forall (l1 l2 : natlist),
  l1 = l2 -> rev l1 = rev l2.
Proof.
  intros l1 l2 H.
  rewrite <- H.
...
Proof.
  intros. destruct l as [|h l'] eqn:E.
  * simpl. reflexivity.
  * simpl. reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros. destruct x as [n] eqn:E. simpl. rewrite eqb_refl. reflexivity. Qed.
(** [] *)
